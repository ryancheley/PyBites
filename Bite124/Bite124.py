...
def get_percentage_female_characters(characters=characters):
    ...
    """
    character_count=0
    female_character_count = 0
    for c in characters:
        if c.sex != "":
            character_count += 1
            if c.sex == "Female Characters":
                female_character_count += 1

    return round(female_character_count / character_count, 4) * 100