...
def group_by_genre(data=movie_excel_file):
    ...
    """
    movies = pd.read_excel(data, names=['Empty1', 'Empty2', 'genres', 'movie'])
    movies.genres = movies.genres.str.split('|')
    movies = movies.drop(columns=['Empty1', 'Empty2'])
    movies = movies.dropna()
    movies = explode(movies, ['genres'])
    result = movies[(movies['genres'] != 'genres') & (movies['genres'] != '(no genres listed)')].groupby(
        ['genres']).agg('count')
    return result.sort_values(by=['movie'], ascending=False)
