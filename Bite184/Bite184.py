from csv import DictReader
import os
from urllib.request import urlretrieve
from collections import defaultdict, Counter

TMP = os.getenv("TMP", "/tmp")
LOGS = 'bite_output_log.txt'
DATA = os.path.join(TMP, LOGS)
S3 = 'https://bites-data.s3.us-east-2.amazonaws.com'
if not os.path.isfile(DATA):
    urlretrieve(f'{S3}/{LOGS}', DATA)


class BiteStats:

    def _load_data(self, data) -> list:
        result = []
        with open(DATA, newline='') as csvfile:
            reader = DictReader(csvfile)
            for row in reader:
                result.append(row)
        return result


    def __init__(self, data=DATA):
        self.rows = self._load_data(data)

    @property
    def number_bites_accessed(self) -> int:
        """Get the number of unique Bites accessed"""
        bite_set = set()
        for item in self._load_data(DATA):
            bite_set.add(item['bite'])
        return len(bite_set)

    @property
    def number_bites_resolved(self) -> int:
        """Get the number of unique Bites resolved (completed=True)"""
        bite_set = set()
        for item in self._load_data(DATA):
            if item['completed'] == 'True':
                bite_set.add(item['bite'])
        return len(bite_set)

    @property
    def number_users_active(self) -> int:
        """Get the number of unique users in the data set"""
        user_set = set()
        for item in self._load_data(DATA):
            user_set.add(item['user'])
        return len(user_set)

    @property
    def number_users_solving_bites(self) -> int:
        """Get the number of unique users that resolved
           one or more Bites"""
        user_set = set()
        for item in self._load_data(DATA):
            if item['completed'] == 'True':
                user_set.add(item['user'])
        return len(user_set)

    @property
    def top_bite_by_number_of_clicks(self) -> str:
        """Get the Bite that got accessed the most
           (= in most rows)"""
        d = defaultdict(int)
        for item in self._load_data(DATA):
            d[item['bite']]+=1
        c = Counter(d)
        return c.most_common()[0][0]

    @property
    def top_user_by_bites_completed(self) -> str:
        """Get the user that completed the most Bites"""
        d = defaultdict(int)
        for item in self._load_data(DATA):
            if item['completed'] == 'True':
                d[item['user']]+=1
        c = Counter(d)
        return c.most_common()[0][0]


c = BiteStats()
print(c.number_bites_accessed)
print(c.number_bites_resolved)
print(c.number_users_active)
print(c.number_users_solving_bites)
print(c.top_bite_by_number_of_clicks)
print(c.top_user_by_bites_completed)