...
def operas_both_at_premiere(guest, composer):
    ...
    composer_born = _get_date(composers[composer].born)
    composer_died = _get_date(composers[composer].died)
    start_date = max(composer_born, guest_born)
    end_date = min(composer_died, guest_died)
    for o in operas:
        for c in composers.keys():
            if start_date <= _get_date(o.date) <= end_date and \
                    c == guest and o.author == composer:
                result.append(o.play)
    return result


o = operas_both_at_premiere('verdi', 'wagner')
print(len(o))