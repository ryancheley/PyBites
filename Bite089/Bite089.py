...
def get_longest_state(data):
    """Receives data, which can be the us_state_abbrev dict or the states
       ...
       of the string"""
    result = sorted(data, key=lambda x: len(x), reverse=True)[0]
    return result
...
def combine_state_names_and_abbreviations(us_state_abbrev=us_state_abbrev,
    ...
    last_10_sorted_states = sorted(states)[-10:]
    first_10_sorted_abbrev = list({k: v for k, v in sorted(us_state_abbrev.items(), key=lambda item: item[1])}.values())[:10]
    return sorted(last_10_sorted_states + first_10_sorted_abbrev)
