...
TWEET2 = ('PyBites My Reading List | 12 Rules for Life - #books that expand the mind! http://pbreadinglist.herokuapp.com/books/TvEqDAAAQBAJ#.XVOriU5z2tA.twitter #psychology #philosophy')
...
def extract_course_times(course=COURSE):
    """Return the course timings from the passed in
       ...
       Return this list.
    """
    return re.findall('[0-9]{2}:[0-9]{2}', course)
...
def get_all_hashtags_and_links(tweet=TWEET):
    ...
    """
    result = []
    potential = re.findall('(\#[a-zA-Z]*)|(http[s]?://(?:[a-zA-Z]|[0-9]|[$-_@.&+]|[!*\(\),]|(?:%[0-9a-fA-F][0-9a-fA-F]))+(\#.[\S]*)?)',tweet)
    for i in potential:
        for j in i:
            if j!= '':
                result.append(j)
    try:
        result.pop(result.index('#.XVOriU5z2tA.twitter'))
    except:
        pass

    return result

g = get_all_hashtags_and_links(TWEET)
print(g)
