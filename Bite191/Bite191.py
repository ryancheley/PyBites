...
def person_max_bmi(data=data):
    ...
    for i in data.split('\n'):
        character_list.append(i.strip().split(','))

    for i in character_list:
        if len(i) > 1:
            char_bmi.append((i[0],round(float(i[2]) / ((int(i[1]) / 100) ** 2),2)))

    result = sorted(char_bmi, key= lambda x: x[1], reverse = True)[0]

    return result


p = person_max_bmi(data)
print(p)
