...
def get_email_details(header: str) -> dict:
    ...
    details = {}
    try:
        details['from'] = re.search(r'From: .*\n', header).group().replace('From: ', '').replace('\n', '')
        details['to'] = re.search(r'To: .*\n', header).group().replace('To: ', '').replace('\n', '')
        details['subject'] = re.search(r'Subject: .*\n', header).group().replace('Subject: ', '').replace('\n', '')
        details['date'] = re.search(r'Date: .*:\d{2}', header).group().replace('Date: ', '')
        return details
